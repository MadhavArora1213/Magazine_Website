...
export const VideoArticleProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (loading) return; // Don't set up interval if already loading
    ...
    const interval = setInterval(() => {
      if (!loading && !document.hidden) {
        ...
        console.log('⏰ Auto-refresh skipped - loading in progress or page hidden');
      }
    }, 30000); // 30 seconds
  ...
  }, [loading]); // Remove fetchVideoArticles dependency to prevent recreating interval
  ...
  }, [loading]); // Remove fetchVideoArticles dependency
